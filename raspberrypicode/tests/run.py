...
# --- THE FIX: ADD TLS FOR AWS IOT ---
if 'certs' in config['mqtt']:
    client.tls_set(
        ca_certs=config['mqtt']['certs']['ca_cert'],
        certfile=config['mqtt']['certs']['cert_file'],
        keyfile=config['mqtt']['certs']['key_file'],
...
# AWS IoT usually doesn't need user/pass if using certs, but wouldnt work without it
client.username_pw_set(config['mqtt']['user'], config['mqtt']['pass'])

# Use the port from your config (8883)
