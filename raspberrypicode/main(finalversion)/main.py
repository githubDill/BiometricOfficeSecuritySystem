...
# Import from your temp.py file
from temp import Si7021
...
def temp_loop():
# Wait a moment for MQTT to connect before starting the loop
    time.sleep(2)
    try:
        sensor = Si7021(
            scl_pin=config['temp_sensor']['scl_pin'], 
            sda_pin=config['temp_sensor']['sda_pin']
        )
        print("✓ Temperature sensor initialized on GPIO", config['temp_sensor']['scl_pin'], "&", config['temp_sensor']['sda_pin'])
    except Exception as e:
        print(f"✗ Temp Sensor Init Error: {e}")
        return
    ...
    while True:
        try:
            ...
            client.publish(config['temp_sensor']['topic'], json.dumps(payload))
        except Exception as e:
            print(f"Reading Error: {e}")
        
        time.sleep(config['temp_sensor'].get('interval', 30))

# --- Startup Logic adjusted for your YAML ---
...
# Adjusted to match your specific YAML keys
...
# Start the separate thread for temp/humidity
